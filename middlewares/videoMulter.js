...
// Configuração de armazenamento para video e thumbnails
const videoStorage = multer.diskStorage({
    ...
    filename: (req, file, cb) => {
        ...
        const uniqueSuffix = Date.now() + "=" + Math.round(Math.random() + 1e9); // Concatena o nome do campo, o sufixo único e a extensão original
        cb(null, `${file.fieldname}-${uniqueSuffix}${path.extname(file.originalname)}`);
    },
});

// Filtro para aceitar apenas videos e imagens
